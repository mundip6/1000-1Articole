...
import { isPackerPending } from "@/lib/packerAuth";
import { verifyPackerOtp } from "../actions";

export default async function PackerTwoFactorPage({
  searchParams,
}: {
    ...
    <main className="flex min-h-screen items-center justify-center bg-neutral-100 px-4 py-12">
      <section className="w-full max-w-md rounded-lg border border-neutral-200 bg-white p-8 shadow-sm">
        <Link href="/" className="mb-8 block text-center">
          ...
          <div className="text-[10px] font-black uppercase tracking-[0.18em] text-neutral-500">Zona Impachetare</div>
        </Link>
        ...
        <form action={verifyPackerOtp} className="space-y-4">
