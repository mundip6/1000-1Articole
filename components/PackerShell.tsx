...
import { logoutPacker } from "@/app/packer/actions";

export default function PackerShell({
  title,
  description,
  children,
}: {
      ...
      <div className="mx-auto max-w-6xl">
        <header className="mb-6 rounded-lg border border-neutral-200 bg-white p-5">
          <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-start">
            <div>
              <p className="text-sm font-bold text-brand">Zona Impachetare</p>
              <h1 className="mt-2 text-3xl font-black">{title}</h1>
              <p className="text-sm text-neutral-500">{description}</p>
            </div>
            <form action={logoutPacker}>
          ...
          <nav className="mt-5 flex flex-wrap gap-2 border-t border-neutral-100 pt-4">
            <Link
              href="/packer/orders"
              className="inline-flex items-center gap-2 rounded-lg bg-brand px-4 py-2 text-sm font-black text-white"
            >
